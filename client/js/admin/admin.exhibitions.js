    ...
    try {
        ...
        if (items.length === 0) {
            ...
            container.innerHTML = items.map(e => {
                const images = Array.isArray(e.images) ? e.images : (typeof e.images === 'string' ? (() => { try { return JSON.parse(e.images); } catch { return []; } })() : []);
                ...
                <div class="exhibition-admin-card">
                    ...
                    <div class="exhibition-admin-actions">
                        <button class="btn btn-ghost btn-sm" onclick="openExhibitionModal(${JSON.stringify(e).replace(/"/g, '&quot;')})">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button class="btn btn-danger btn-sm" onclick="deleteExhibition(${e.id}, '${esc(e.title)}')">
...
function openExhibitionModal(exhibition = null) {
    ...
    if (exhibition) {
        ...
        const images = Array.isArray(exhibition.images) ? exhibition.images : (typeof exhibition.images === 'string' ? (() => { try { return JSON.parse(exhibition.images); } catch { return []; } })() : []);
...
// ══════════════════════════════════════════
//  FORM TAB SWITCHING (shared utility)
// ══════════════════════════════════════════
function switchFormTab(tabId, formId) {
    const form = document.getElementById(formId);
    if (!form) return;
    form.querySelectorAll('.form-tab-btn').forEach(b => b.classList.remove('active'));
    form.querySelectorAll('.form-tab-pane').forEach(p => p.classList.remove('active'));
...
// ══════════════════════════════════════════
//  DROP ZONE SETUP (called on modal open)
// ══════════════════════════════════════════
...
async function saveExhibition(e) {
    ...
    try {
        ...
        formData.append('title', document.getElementById('exhTitle').value.trim());
        ...
        formData.append('is_active', document.getElementById('exhActive').checked);
...
async function deleteExhibition(id, title) {
    ...
    try {
        ...
        loadAdminExhibitions();
    } catch (err) { Toast.error(err.message || 'Failed to delete exhibition.'); }
}
